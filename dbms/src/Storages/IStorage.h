...
public:
    ...
    virtual void clearColumnInPartition(const ASTPtr & partition, const Field & column_name, const Settings & settings)
    {
    ...
    /** Run the query (DROP|DETACH) PARTITION.
      */
    virtual void dropPartition(const ASTPtr & query, const ASTPtr & partition, bool detach, const Settings & settings)
    {
    ...
    /** Run the ATTACH request (PART|PARTITION).
      */
    virtual void attachPartition(const ASTPtr & partition, bool part, const Settings & settings)
    {
    ...
    /** Run the FETCH PARTITION query.
      */
    virtual void fetchPartition(const ASTPtr & partition, const String & from, const Settings & settings)
    {
    ...
    /** Run the FREEZE PARTITION request. That is, create a local backup (snapshot) of data using the `localBackup` function (see localBackup.h)
      */
    virtual void freezePartition(const ASTPtr & partition, const String & with_name, const Settings & settings)
    {
    ...
    /** Perform any background work. For example, combining parts in a MergeTree type table.
      ...
      */
    virtual bool optimize(const ASTPtr & query, const ASTPtr & partition, bool final, bool deduplicate, const Settings & settings)
    {
